...
export const VOICE_STYLES = [
  ...
  },
  {
    id: "elderly",
    label: "나이든",
    // Deliberately does NOT ask for slower pacing — that measurably
    // lengthens the synthesized audio and pushes it further past its time
    // slot (worse atempo-capped drift/sync), confirmed by real user
    // feedback. Age comes through via voice quality only, at a normal pace.
    instructions:
      "You are voicing an elderly person in their 70s-80s. This must sound clearly, unmistakably old — not just calm. Use a frail, slightly shaky, raspy voice quality with a lower, more worn timbre, the way a grandparent speaks, but at a normal speaking pace.",
  },
  {
    id: "gruff",
    label: "걸걸한",
