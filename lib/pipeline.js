...
// timeline, unaffected by any dubbed-audio drift) so the original spoken
// dialogue is never audible, while leaving everything else (sound effects,
// hits, background music, ambience) untouched at full volume. One `volume`
// filter stage per segment chained in a single -af pass — still a single
// real input, no filter_complex.
async function muteOriginalDuringDialogue(originalAudioPath, translatedSegments, outputPath) {
  const filters = translatedSegments.map(
    (s) => `volume=0:enable='between(t\\,${s.start.toFixed(3)}\\,${s.end.toFixed(3)})'`
  );
...
async function translateAndAnalyzeSegments(segments, targetLanguage, videoPath, dir) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    content.push({ type: "text", text: `Segment ${i}: "${seg.text}"` });
    try {
...
async function blurSubtitleRegion(videoPath, outputPath) {
  await execFileAsync(
    ...
    [
      "-y", "-nostdin", "-i", videoPath,
      "-filter_complex",
      "split[base][b];[b]crop=iw:ih*0.15:0:ih*0.76,boxblur=12:3[blurred];[base][blurred]overlay=0:main_h*0.76",
      "-an",
