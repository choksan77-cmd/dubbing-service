...
// Requesting word-level granularity alongside segment-level measurably
// improves the accuracy of the returned SEGMENT timestamps too, even though
// the word array itself isn't used here — segment-only requests come from a
// coarser internal alignment. Confirmed on a real fast-dialogue test video:
// a segment-only request placed a line at 10.6-11.72s when the audio (and
// the source video's own on-screen subtitle) showed it was actually still
// being spoken past 13s — a 1-2s timestamp error that fed inaccurate
// windows into every downstream step (TTS slot duration, subtitle timing,
// original-audio mute windows, the review-stage caption).
...
// Segments-per-request cap for translateAndAnalyzeSegments. One inline
// image per segment (even at detail:"low") adds up fast across a whole
// video — a single request covering a few dozen segments measurably hit a
// real account's tokens-per-minute limit (a 429 "Request too large" on a
// video with far more segments than earlier test clips). Chunking bounds
// each request's size regardless of video length; chunks are sent
// sequentially (already the case, one await per chunk) which also keeps
// per-minute throughput well under typical TPM limits.
const CAST_CHUNK_SIZE = 8;

function buildCastInstructions(targetLanguage, knownCast) {
  const castNote = knownCast.length
    ? `Established cast so far (from earlier lines in this same video): ${knownCast
        .map((c) => `"${c.speaker}" (${c.gender}, ${c.age})`)
        .join(
          ", "
        )}. Reuse these EXACT labels when the same person speaks again in this batch. Only introduce a new label if the dialogue or imagery genuinely shows a person not in this list. `
    : "";
  ...
  return (
    ...
    `same person, including labels from the established cast note above if given). This batch is only part of a ` +
    `longer scene — you won't see the whole conversation, so lean more heavily on the established cast note and on ` +
    `the image than you would if you could see everything. Short or generic lines ("what's going on", reactions) ` +
    `give almost no textual signal on their own — for those, lean on a clear image if you have one, otherwise ` +
    `default to keeping the SAME speaker as the immediately preceding line rather than guessing a new one. Do not ` +
    `invent a new speaker label unless the dialogue or imagery genuinely demands it. Also infer: gender ("male" or ` +
    `"female", best guess for voice casting); age ("child", "young", "middle", or "elderly", best guess); emotion ` +
    `(one of "neutral", "calm", "angry", "crying", "happy", "sad" — the emotional tone of this specific line). ` +
...
async function translateAnalyzeChunk(
  chunkSegments,
  offset,
  targetLanguage,
  videoPath,
  ...
  const content = [{ type: "text", text: buildCastInstructions(targetLanguage, knownCast) }];
  ...
  for (let i = 0; i < chunkSegments.length; i++) {
    ...
    if (includeImages) {
      try {
        const framePath = path.join(dir, `speaker_frame_${offset + i}.jpg`);
        ...
        console.error(`Frame extraction failed for segment ${offset + i}, continuing text-only:`, error);
      }
    }
  }
  ...
  content.push({
    type: "text",
    text: `Return the JSON object now, with exactly ${chunkSegments.length} entries in "segments", matching the segments above in order.`,
  });
  ...
  if (!rawContent) {
    // gpt-4o can return null content instead of throwing on a refusal —
    // this app's typical source content (short-form dramas) includes
    // violence/distress imagery that can trip moderation on a frame. Retry
    // once text-only (the images are the likely trigger) before giving up
    // with a clear error instead of crashing on `null.segments`.
    if (includeImages) {
      console.error(`Chunk at offset ${offset} got empty content with images, retrying text-only`);
      return translateAnalyzeChunk(chunkSegments, offset, targetLanguage, videoPath, dir, knownCast, false);
    }
    throw new Error(
      `GPT-4o returned an empty response for segments ${offset}-${offset + chunkSegments.length - 1} ` +
        `even text-only (likely a content moderation refusal).`
    );
  }
...
// Translates AND casts the dialogue, now WITH a video frame per segment
// alongside its text — text-only speaker inference (no visual signal at
// all) reliably mis-assigns short/generic lines ("what's going on?") when
// there's no textual clue which of several people said it. Feeding GPT-4o
// the actual frame lets it use who's visibly on screen, not just guess
// from the words. Frame extraction failures degrade gracefully (that
// segment just gets analyzed text-only instead of failing the job).
//
// Processed in fixed-size chunks (see CAST_CHUNK_SIZE) rather than one
// request for the whole video — see the comment there for why. A running
// "established cast" list is threaded through each chunk so speaker labels
// stay consistent across chunk boundaries even though no single request
// sees the whole scene at once.
async function translateAndAnalyzeSegments(segments, targetLanguage, videoPath, dir, jobId) {
  const analyzed = [];
  const knownCast = [];

  await prisma.dubJob.update({
    where: { id: jobId },
    data: { progressCurrent: 0, progressTotal: segments.length },
  });

  for (let offset = 0; offset < segments.length; offset += CAST_CHUNK_SIZE) {
    const chunk = segments.slice(offset, offset + CAST_CHUNK_SIZE);
    const chunkResults = await translateAnalyzeChunk(chunk, offset, targetLanguage, videoPath, dir, knownCast);
    analyzed.push(...chunkResults);
    await prisma.dubJob.update({
      where: { id: jobId },
      data: { progressCurrent: analyzed.length },
    });

    for (const r of chunkResults) {
      const speaker = typeof r?.speaker === "string" && r.speaker.trim() ? r.speaker.trim() : null;
      if (speaker && !knownCast.some((c) => c.speaker === speaker)) {
        knownCast.push({
          speaker,
          gender: r?.gender === "female" ? "female" : "male",
          age: ["child", "young", "elderly"].includes(r?.age) ? r.age : "middle",
        });
      }
    }
  }
...
function parseDimensionsFromOutput(text) {
  const match = /Video:.*?,\s*(\d+)x(\d+)/.exec(text || "");
  if (!match) return null;
  return { width: Number(match[1]), height: Number(match[2]) };
}

// Same single-input probe pattern as getMediaDuration. Needed so libass
// (via the `subtitles` filter) doesn't fall back to its own default script
// resolution — without an explicit original_size matching the real video,
// libass scales FontSize/MarginV against the wrong canvas, which is
// especially visible on non-standard (vertical, 9:16) frame sizes like
// these short-form sources: a MarginV tuned by eye at one resolution landed
// wildly off-position at the actual video's resolution.
async function getMediaDimensions(filePath) {
  try {
    ...
    return parseDimensionsFromOutput(stderr);
  } catch (error) {
    return parseDimensionsFromOutput(error.stderr);
  }
}
...
// Cuts a source video into fixed-length pieces so each piece can go through
// the pipeline as its own independent job — sidesteps Whisper's 25MB audio
// upload limit for long videos without needing to re-stitch anything
// afterward (each chunk's dub output stands alone). `-c copy` (no
// re-encode) for speed on potentially large files; this snaps cut points to
// the nearest keyframe rather than an exact sample-accurate boundary, which
// is fine here since chunk boundaries aren't meaningful narrative moments.
// Single real input per call, sequential — same safe ffmpeg pattern as
// everything else in this file.
export const CHUNK_SECONDS = 150; // 2m30s

export async function splitVideoIntoChunks(videoPath, outDir, chunkSeconds = CHUNK_SECONDS) {
  const totalDuration = await getMediaDuration(videoPath);
  const chunkCount = Math.max(1, Math.ceil(totalDuration / chunkSeconds));
  if (chunkCount === 1) return [videoPath];

  const chunkPaths = [];
  for (let i = 0; i < chunkCount; i++) {
    const start = i * chunkSeconds;
    const outPath = path.join(outDir, `chunk_${i}.mp4`);
    await execFileAsync(
      ffmpegPath,
      [
        "-y", "-nostdin",
...
// video's length.
async function assembleSegmentAudio(translatedSegments, characters, dir, totalDurationSeconds, jobId) {
  if (jobId) {
    await prisma.dubJob.update({
      where: { id: jobId },
      data: { progressCurrent: 0, progressTotal: translatedSegments.length },
    });
  }
  ...
  const rawPaths = await mapWithConcurrency(translatedSegments, 4, async (seg, i) => {
    ...
    completedCount += 1;
    if (jobId) {
      await prisma.dubJob.update({ where: { id: jobId }, data: { progressCurrent: completedCount } });
    }
    return rawPath;
  });
...
async function burnInSubtitles(videoPath, srtPath, outputPath, dimensions) {
  // Styled to match a tight caption "pill" (opaque background box sized to
  // the text, BorderStyle=3) sitting inside the blurred band, rather than a
  // large bold line anchored far below it — closer to what CapCut/Typecast
  // produce and requested directly by the user after a side-by-side
  // comparison. MarginV is measured from the bottom for Alignment=2 and is
  // tuned (as a fraction of real video height, see below) so the box lands
  // inside blurSubtitleRegion's band (0.63-0.79 of frame height).
  //
  // original_size is required here: without it, libass scores
  // FontSize/MarginV against its own default script resolution instead of
  // the actual video's pixel dimensions, which on a non-standard vertical
  // (9:16) frame put the box near the eyes instead of near the bottom band
  // — confirmed visually on a real test video before this was added.
  const width = dimensions?.width || 406;
  ...
  const vf = [
    ...
    `force_style='FontName=${FONT_FAMILY_NAME},Bold=1,FontSize=22,PrimaryColour=&H00FFFFFF,BackColour=&H99000000,BorderStyle=3,Outline=6,Shadow=0,Alignment=2,MarginV=50'`,
  ].join(":");
...
async function muxFinalVideo({ videoPath, audioPath, srtPath, outputPath }) {
  ...
  await burnInSubtitles(blurredPath, srtPath, subtitledPath, dimensions);
...
export async function runTranscribeAndTranslate(jobId) {
  ...
  try {
    ...
    const analyzedSegments = await translateAndAnalyzeSegments(segments, job.targetLanguage, videoPath, dir, jobId);
...
export async function runDubAndMux(jobId) {
  ...
  try {
    ...
    await updateStatus(jobId, "muxing", { progressCurrent: null, progressTotal: null });
