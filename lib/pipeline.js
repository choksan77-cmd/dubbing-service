...
const FONT_FILE = path.join(process.cwd(), "assets", "fonts", "NotoSansKR-Regular.ttf");
...
// Breaks a line into a few shorter lines so it doesn't run off-screen —
// drawtext (unlike the libass `subtitles` filter) does no auto-wrapping.
...
// Burns the translated subtitles directly into the video pixels instead of
// a soft mov_text track (which most players never show by default, so
// viewers just saw the untouched original-language hardcoded subtitles).
// Uses `drawtext` with a direct fontfile= path (freetype only, no
// name-based font lookup) rather than the `subtitles`/libass filter —
// libass's font matching depends on the container's font-config backend,
// which silently failed to resolve our bundled font on Railway's Linux
// binary even with fontsdir set (rendered no text at all, no error). A
// direct file path sidesteps that entirely and is what we already verified
// renders Korean correctly. One drawtext stage per segment, chained in a
// single -vf pass (still just one real input, no filter_complex).
async function burnInSubtitles(videoPath, translatedSegments, timeline, dir, outputPath) {
  const filters = [];

  for (let i = 0; i < translatedSegments.length; i++) {
    const text = (translatedSegments[i].translatedText || "").trim();
    if (!text) continue;

    const textFilePath = path.join(dir, `subtitle_text_${i}.txt`);
    fs.writeFileSync(textFilePath, wrapSubtitleText(text), "utf8");

    const start = timeline[i].actualStart.toFixed(3);
    const end = timeline[i].actualEnd.toFixed(3);

    filters.push(
      "drawtext=" +
        [
          `fontfile='${escapeFfmpegPath(FONT_FILE)}'`,
...
async function muxFinalVideo({ videoPath, audioPath, translatedSegments, timeline, dir, outputPath }) {
  ...
  await burnInSubtitles(blurredPath, translatedSegments, timeline, dir, subtitledPath);
